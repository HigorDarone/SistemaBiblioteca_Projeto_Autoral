...
- **Carrinho (Web API) — concluído.** `CarrinhoController` expõe adicionar item, listar itens (com o total já calculado, numa resposta combinada `ListarCarrinhoResponse`), remover item, remover uma unidade de um item (`PATCH`, por ser uma atualização parcial, diferente do `DELETE` usado pra remover o item inteiro) e finalizar o pedido. Como a API ainda não tem autenticação de verdade (JWT fica pra depois), o `usuarioId` é passado explicitamente na URL de cada requisição — solução temporária e conhecida, não seria segura numa aplicação em produção. A lógica de achar (ou criar) o carrinho de um usuário foi centralizada num método privado (`BuscarOuCriarCarrinho`) reaproveitado por todos os endpoints, em vez de repetida em cada um. Erros de estado inválido (como finalizar um carrinho vazio) seguem o mesmo padrão já estabelecido no cadastro: `InvalidOperationException` → `409 Conflict`.
- Ainda falta: endpoint de histórico de pedidos (listar `Pedido`s já finalizados de um usuário).
